...
export class Object3D extends Struct({ center: Point, radius: Field }) {
  static fromObjectAndTranslationMatrices(
    ...
    translationMatrices: {
      ...
      translationToPositiveCoordsMatrix: THREE.Matrix4;
    },
  ) {
    const scale = 1000000;
    ...
    return new Object3D({
      center: Point.fromCoords(
        Field(Math.round(center.x * scale)),
...
export class Box extends Struct({ a: Point, b: Point }) {
  ...
  static fromVerticesAndTranslationMatrices(
    ...
    v.forEach((vertex) => {
      const distance = vertex.length(); // Get the distance from origin
    ...
    return new Box({
      a: Point.fromCoords(
        Field(Math.round(closestVertex.x * scale)),
        Field(Math.round(closestVertex.y * scale)),
        Field(Math.round(closestVertex.z * scale)),
      ),
      b: Point.fromCoords(
        Field(Math.round(farthestVertex.x * scale)),
